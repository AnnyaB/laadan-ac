...
def _actor_critic_common_setup(benchmark, config, use_cost_head):
    ...
    """
    Shared setup for VOAC, LAADAN-AC and LAADAN-AC-RC.
    """
...
def constrained_checkpoint_is_better(metrics, best_metrics, cost_budget, tolerance=1e-8):
    """
    Select checkpoints using strict constrained model-selection logic.

    For LAADAN-AC-PD, the constraint is checked using both:
    - primary deterministic inadmissibility_rate
    - auxiliary stochastic soft_inadmissibility_rate

    The selected checkpoint is feasible only if both are within budget.
    This is stricter and more scientifically consistent with primal-dual CMDP training.
    """
...
def exact_policy_metrics_from_torch(benchmark, policy_tensor):
    """
    Evaluate a PyTorch policy tensor using the benchmark's exact finite-horizon evaluator.
    """
...
def risk_adjusted_logits(
    ...
    """
    Build robust LAADAN-AC action logits.

    This keeps the actor's learned logits active, but reshapes them using:
    - lower-risk reward/cost trade-off,
    - Lagrangian cost pressure,
    - benchmark-defined immediate inadmissibility cost,
    - expert-support penalty,
    - smoothness penalty.

    The reward and cost scores are detached here so that this helper shapes the
    actor distribution without updating critic parameters through the actor step.
    """
...
def certified_greedy_policy(
    ...
    """
    Deterministic certified action-nudging policy.

    First, choose the highest-energy action among certified-safe actions:
    - benchmark immediate cost is admissible,
    - predicted cost-to-go is below threshold,
    - expert/support probability is positive.

    If no action satisfies all three, fall back to the best benchmark-admissible
    action. If even that is missing, choose the lowest-risk action overall.
    """
...
def reward_lcb_from_twin_q(q1, q2, rho_reward=1.0):
    """
    Lower-confidence reward value from twin reward critics.

    Q_LCB = mean(Q1,Q2) - rho * std(Q1,Q2)
    """
...
def cost_ucb_from_ensemble(cost_raw, rho_cost=1.0):
    """
    Upper-confidence cost value from one or more cost heads.

    If cost_raw has shape [S,A], it is treated as a single-head cost critic.
    If cost_raw has shape [K,S,A], it is treated as an ensemble.

    Q_C_UCB = mean(Q_C) + rho * std(Q_C)
    """
    if cost_raw.dim() == 2:
...
def update_lagrange_multiplier(value, measured_cost, budget, lr, max_value):
    """
    Projected primal-dual ascent update.

    lambda <- clip(lambda + lr * (measured_cost - budget), 0, max_value)
    """
    return max(
        0.0,
        min(
...
def expected_policy_cost(policy, cost_matrix):
    """
    Mean over states of expected per-state cost under policy.
    """
...
def risk_adjusted_logits_rc(
    ...
    """
    Robust constrained LAADAN-AC action logits.

    adjusted_logits =
        actor_logits
        + reward lower-confidence value
        - lambda_inad * cost upper-confidence value
        - benchmark inadmissibility barrier
        - expert-support pressure
        - smoothness pressure
        - uncertainty pressure

    This keeps the actor trainable while shaping its distribution through
    robust constrained safety terms.
    """
...
def certified_greedy_policy_rc(
    ...
    """
    Certified action-nudging policy.

    First chooses the highest-energy action among certified actions:
    - benchmark immediate cost is admissible;
    - cost UCB is below threshold;
    - uncertainty is below threshold;
    - expert/support probability is positive.

    If no such action exists, it falls back to the best benchmark-admissible action.
    If even that is unavailable, it chooses the lowest-risk action.
    """
...
def train_laadan_ac_primal_dual(benchmark, seed, results_dir, config):
    """
    Train LAADAN-AC-RC v1.

    Robust-Constrained LAADAN-AC v1 includes:
    - reward lower-confidence bound from twin reward critics;
    - cost upper-confidence bound from a cost critic ensemble;
    - multi-lambda constraints for inadmissibility, support, smoothness and uncertainty;
    - expert-policy support proxy;
    - certified deterministic action nudging.

    This is not a proof of SOTA by itself. It is the architectural version that
    can later be compared against BCQ, BEAR, CQL, CPQ, COptiDICE, FISOR and SafeFQL.
    """
